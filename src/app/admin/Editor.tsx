...
export const Editor = ({ data: initdata }: { data: 데이터[] }) => {
    ...
    return (
        <div className="flex">
            <textarea ref={inputref} id="story" name="story" rows={5} cols={33} onChange={(e) => { console.log(inputref.current?.value) }}>
                {AtoB(data)}
            </textarea>
            <button onClick={() => setData(initdata)} className="self-start cursor-pointer">적용</button>
            <button onClick={() => {
                saveTemplateAsFile(BtoA(inputref.current?.value ?? ""))
            }} className="self-start cursor-pointer">저장</button>
